...
def heart_model(heart_gdf):
    ...
    # Add the legend to the map
    macro = MacroElement()
    macro._template = Template(template7)
    macro.add_to(m)
...
def load_data(park_name):
    if park_name == "북한산":
        ...
        shelter = gpd.read_file("./data/지리산 안전쉼터.geojson")
        aed = gpd.read_file("./data/지리산 AED.geojson")
        fall_gdf = gpd.read_file("./data/지리산 추락사고(100)_gdf.geojson")
        heart_gdf = gpd.read_file("./data/지리산 심장사고(100)_gdf.geojson")
        park_watch = smartwatch[smartwatch['국립공원'] == '지리산']
    else:
        # 기본값 또는 오류 처리
...
if button:
    with st.spinner('분석 중입니다. 잠시만 기다려주세요!'):
        ...
        with col[1]:
            ...
            if selected_national_park in ['북한산', '설악산', '무등산', '지리산']:
            ...
            if selected_national_park in ['북한산', '설악산', '무등산', '지리산']:
                with tabs[tab_index]:
                    ...
                    folium_static(m3)
                    st.write("설치 시급지점이란?")
                    st.write("왼쪽 사이드바에서 설정한 이격거리로 모든 사고발생확률 95% 이상인 구간 중심점과 모든 기존 안전쉼터와의 거리를 재고,")
                    st.write("설정한 거리보다 더 멀리 떨어진 지점을 설치 시급지점으로 볼 수 있어요.")
